...
import {
  M_CREATE_CAPITAL_TYPE,
  M_DELETE_CAPITAL_TYPE,
  M_UPDATE_CAPITAL_TYPE,
  Q_GET_CAPITAL_TYPE_TREES,
} from '@/gql';
...
export default () => {
  ...
  const { loading, data, refetch, client } = useQuery(Q_GET_CAPITAL_TYPE_TREES, {
    notifyOnNetworkStatusChange: true,
  });
  const [updateCapitalType] = useMutation(M_UPDATE_CAPITAL_TYPE, {
    update: (cache, { data }) => {
      if (data) {
        refetch();
        setSelectedNode(data.updateCapitalType);
        message.success('保存成功');
      }
    },
  });
  const [createCapitalType] = useMutation(M_CREATE_CAPITAL_TYPE, {
    update: (cache, { data }) => {
      if (data) {
        refetch();
  ...
  const [deleteCapitalType] = useMutation(M_DELETE_CAPITAL_TYPE, {
    update: (cache, { data }) => {
      if (data) {
        refetch();
        setCheckedKeys([]);
  ...
  const { capitalTypeTrees } = data;

  if (!capitalTypeTrees) return <Skeleton />;
  ...
  const actions = [
    ...
    { name: '新增', icon: 'file-add', action: () => setPanelVisible(true) },
    {
      ...
      icon: 'delete',
      action: () => {
        deleteCapitalType({ variables: { ids: checkedKeys.join(',') } });
      },
  ...
  return (
    <PageHeaderWrapper>
      ...
      <StandardRow bodyStyle={{ marginTop: 10 }}>
        <Row>
          ...
          <Col lg={15} offset={1}>
            ...
            {selectedNode ? (
              <InfoForm isUpdate={true} mutation={updateCapitalType} target={selectedNode} />
            ) : (
              ''
            )}
          </Col>
      ...
      >
        <InfoForm mutation={createCapitalType} target={selectedNode} />
      </StandardPanel>
    </PageHeaderWrapper>
  );
};
