...
import { M_CREATE_AMOUNT, M_DELETE_AMOUNT, M_UPDATE_AMOUNT, Q_GET_AMOUNT_TREES } from '@/gql';
...
export default () => {
  ...
  const { loading, data, refetch, client } = useQuery(Q_GET_AMOUNT_TREES, {
    notifyOnNetworkStatusChange: true,
  });
  const [updateAmount] = useMutation(M_UPDATE_AMOUNT, {
    update: (cache, { data }) => {
      if (data) {
        refetch();
        setSelectedNode(data.updateAmount);
        message.success('保存成功');
      }
    },
  });
  const [createAmount] = useMutation(M_CREATE_AMOUNT, {
    update: (cache, { data }) => {
      if (data) {
        refetch();
  ...
  const [deleteAmount] = useMutation(M_DELETE_AMOUNT, {
    update: (cache, { data }) => {
      if (data) {
        refetch();
        setCheckedKeys([]);
  ...
  const { amountTrees } = data;

  if (!amountTrees) return <Skeleton />;
  ...
  const actions = [
    ...
    { name: '新增', icon: 'file-add', action: () => setPanelVisible(true) },
    {
      ...
      icon: 'delete',
      action: () => {
        deleteAmount({ variables: { ids: checkedKeys.join(',') } });
      },
  ...
  return (
    <PageHeaderWrapper>
      ...
      <StandardRow bodyStyle={{ marginTop: 10 }}>
        <Row>
          ...
          <Col lg={15} offset={1}>
            ...
            {selectedNode ? (
              <InfoForm isUpdate={true} mutation={updateAmount} target={selectedNode} />
            ) : (
              ''
            )}
          </Col>
      ...
      >
        <InfoForm mutation={createAmount} target={selectedNode} />
      </StandardPanel>
    </PageHeaderWrapper>
  );
};
