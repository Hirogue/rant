...
@connect(({ usersDetail, loading }) => ({
  usersDetail,
  loading: loading.effects['usersDetail/fetchAdvanced'],
}))
class Detail extends Component {
  ...
  componentDidMount() {
    ...
    dispatch({
      type: 'usersDetail/fetchAdvanced',
    });
  }
  ...
  render() {
    ...
    const { usersDetail, loading } = this.props;
    const { advancedOperation1, advancedOperation2, advancedOperation3 } = usersDetail;
    const contentList = {
      tab1: (
        <Table
          pagination={false}
...
export default withRouter(props => {
  ...
  return (
    <PageHeaderWrapper
      ...
      extra={action}
      className={styles.pageHeader}
      content={<PageHeaderContent user={user} />}
      extraContent={
        <div className={styles.moreInfo}>
          <Statistic title="状态" value={UserStatusMaps[user.status]} />
        </div>
      }
    >
      <div className={styles.main}>
