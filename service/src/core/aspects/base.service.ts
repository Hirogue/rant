...
export abstract class BaseService<T> extends CrudService<T> {
  ...
  private validateHasColumn(column: string) {
    if (column.indexOf('.') !== -1) {
      ...
      const noColumn = !(this.entityRelationsHash[relation]
        .columns as string[]).find(o => o === column);
  ...
  private getRelationMetadata(field: string) {
    try {
      ...
      } = relations.find(o => o.propertyName === target);
