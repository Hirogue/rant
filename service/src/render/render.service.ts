import { HttpServer, Injectable, InternalServerErrorException } from '@nestjs/common';
...
export class RenderService {
    ...
    public bindHttpServer(server: HttpServer) {
        ...
        server.render = (response: any, view: string, data: any) => {
            ...
            if (req && res && renderer) {
                req.viewData = req.viewData ? req.viewData : {};
                return renderer(req, res, getViewPath(view), { ...req.viewData, ...data });
            } else if (!renderer) {
