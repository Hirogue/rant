						...
						<Col>
							<Form.Item {...formItemLayout} label={'行业类型'}>
								{getFieldDecorator('industry', {
									initialValue: target ? target.industry ? target.industry.map(item => item.id) : null : null,
									rules: [
							...
							<Form.Item {...formItemLayout} label={'投资金额'}>
								{getFieldDecorator('amount', {
									...
									rules: [
										{ required: true, message: '请填写投资金额' }
									]
								})(<InputNumber disabled={disabled} min={1} style={{ width: 200 }} placeholder="请填写投资金额" />)}
								{'  '}万元
							</Form.Item>
							...
							<Form.Item {...formItemLayout} label={'投资地区'}>
								{getFieldDecorator('invest_area', {
									initialValue: target ? target.invest_area ? target.invest_area.map(item => item.id) : null : null,
									rules: [
								...
								<>
									...
									<Form.Item {...formItemLayout} label={'投资阶段'}>
										{getFieldDecorator('stage', {
											initialValue: target ? target.stage ? target.stage.map(item => item.id) : null : null,
											rules: [
												{ required: true, message: '请选择投资阶段' }
											]
										})(<Checkbox.Group
											disabled={disabled}
									...
									<Form.Item {...formItemLayout} label={'投资期限'}>
										{getFieldDecorator('term', {
											...
											rules: [
												{ required: true, message: '请填写投资期限' }
											]
										})(<InputNumber disabled={disabled} min={1} style={{ width: 200 }} placeholder="请填写投资期限" />)}
										{'  '}年
									</Form.Item>
									<Form.Item {...formItemLayout} label={'投资类型'}>
										{getFieldDecorator('invest_type', {
											initialValue: target ? target.invest_type ? target.invest_type.map(item => item.id) : null : null,
											rules: [
												{ required: true, message: '请选择投资类型' }
											]
										})(<Checkbox.Group
											disabled={disabled}
								...
								<>
									<Form.Item {...formItemLayout} label={'最低回报要求'}>
										{getFieldDecorator('return', {
											...
											rules: [
												{ required: true, message: '请填写最低回报要求' }
											]
										})(<Input disabled={disabled} style={{ width: 500 }} placeholder="请填写最低回报要求" />)}
									</Form.Item>
									...
									<Form.Item {...formItemLayout} label={'抵质押物类型'}>
										...
										})(<Input disabled={disabled} style={{ width: 500 }} placeholder="请填写抵质押物类型" />)}
									</Form.Item>
									<Form.Item {...formItemLayout} label={'抵质押物折扣率'}>
										{getFieldDecorator('discount', {
											initialValue: target ? target.discount : null
										})(<InputNumber disabled={disabled} min={1} max={10} step={0.1} style={{ width: 200 }} placeholder="请填写投资金额" />)}
										{'  '}折
							</Form.Item>
								</>
								: null}
