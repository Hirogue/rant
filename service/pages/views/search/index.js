...
import { Q_SEARCH_PROJECT, Q_SEARCH_CAPITAL, Q_SEARCH_PRODUCT, Q_SEARCH_ARTICLE } from '../../gql'
...
const categoryMap = {
	product: {
		...
		filter: (keyword) => `&filter[]=["name||cont||${keyword}","introduction||cont||${keyword}"]&filter=is_published||eq||true`
	},
	project: {
		...
		filter: (keyword) => `&filter[]=["title||cont||${keyword}","info||cont||${keyword}"]&filter[]=status||in||waiting,following,checked,finished`
	},
	finance: {
	...
	news: {
		...
		filter: (keyword) => `&filter[]=["title||cont||${keyword}","text||cont||${keyword}"]&filter=is_published||eq||true`
	}
}
...
export default withRouter((props) => {
	...
	const { loading, fetchMore, data } = useQuery(target.gql, {
		fetchPolicy: "no-cache",
		notifyOnNetworkStatusChange: true,
		client: client,
		variables: {
			queryString: buildingQuery(target.variables) + target.filter(keyword)
		}
	});

	const toRenderList = {
		product: (list) => list.map((item) => (
			<a
		...
		news: (list) => list.map(item => (
			...
			>
				<div className="item">
					<a className="title" href={`/${searchCategory}/detail?id=${item.id}`}> {item.title} </a>
					<p className="text"> {item.summary} <a className="more" href={`/${searchCategory}/detail?id=${item.id}`}> [详情] </a> </p>
					<ul>
						<li>
							<IconFont className="iconfont" type="icon-shijian1" />
							{moment(item.publish_at).format('YYYY-MM-DD')}
	...
	const onPageChange = (page, pageSize) => {
		let variables = Object.assign(target.variables, {
			page: page,
			limit: pageSize,
		});
		fetchMore({
			variables: {
				queryString: buildingQuery(variables) + target.filter(keyword)
			},
			...
			<div className="search-result">
				...
				<div className="content-main">
					...
					{loading ? (
						...
						!!list && list.length > 0 ? (
							toRenderList[searchCategory](list)
						) : (
							<div className="item">
					...
					<PageMod current={current} total={data && data.result ? data.result.total : 0} pageSize={10} onChange={onPageChange} />
				</div>
			</div>
		</BaseLayout>
	)
